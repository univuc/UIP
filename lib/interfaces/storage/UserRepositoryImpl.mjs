...
class UserRepositoryImpl extends UserRepository {
    ...
    async _saveUser(newUser) {
        try {
            if (!await this._assertUserNotExists(newUser)) {
                return false;
            }
    ...
    async _assertUserNotExists(user) {
        const exists = await this._checkIfUserExists(user);
        if (exists) {
    ...
    async _updateUser(user) {
        try {
            if (!await this._assertUserExists(user)) {
                return null;
            }
    ...
    async _assertUserExists(user) {
        const exists = await this._checkIfUserExists(user);
        if (!exists) {
            logger.warn(`User ${user.id} does not exist!`);
            return false;
        }
